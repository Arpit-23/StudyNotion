...
exports.createCourse = async (req, res) =>{
    ...
    if (
      ...
      if (!status || status === undefined) {
        ...
        //add the new course in to the user schema of intructor
  ...
  // console.log("inside getallcourses")
    try{
        const allCourses = await Course.find(
            ...
            .exec();
            // console.log("given " ,allCourses)
        return res.status(200).json({
            success:true,
            message:"All coursse data fetch successfully",
            data:allCourses,
        })
    }
    catch(error){
...
exports.editCourseDetails = async (req, res) =>{
    try{
        ...
        //if thumbnail is found so update it
        if(req.files){
            const thumbnail = req.files.thumbnailImage;
            const thumbnailImage = await uploadImageToCloudinary(thumbnail, process.env.FOLDER_NAME);
        ...
        //update  these field which are present in req.body
